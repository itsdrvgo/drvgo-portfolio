...
tags: ["trpc", "nextjs", "typescript", "react", "tutorial"]
...
-   Now that we've the database setup, let's setup the tRPC server. Create a folder called **trpc** inside the **lib** folder and create a file called **context.ts** inside it. And paste the following code in it.
    ...
    You can read more about what **Context** is in the <Link href="https://trpc.io/docs/server/context">tRPC Documentation</Link>.
...
-   If you have any authentication layer or you want to use any middleware in your routes, you can have something like this,
    ...
    Now, all routes with the **protectedProcedure** procedure will be protected by the **isAuth** middleware. You can read more about middleware in the <Link href="https://trpc.io/docs/server/middleware">tRPC Documentation</Link>.
...
-   Now, inside the **route.ts** file, paste the following code.
    ...
    Even though tRPC provides a built-in Next.JS adapter, we can still use the native **fetch** adapter. You can read more about it in the <Link href="https://trpc.io/docs/server/adapters/fetch">tRPC Documentation</Link>.
