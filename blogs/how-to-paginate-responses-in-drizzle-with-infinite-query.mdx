...
<Image type="local" location="/images/pagination_definition.jpg" />
...
```bash
pnpm add drizzle-orm drizzle-pagination drizzle-zod postgres
```
...
```bash
pnpm add @mantine/hooks @paralleldrive/cuid2
```

## Setting up the Database

Now, let's setup the database. For this tutorial, we'll be using **PostgreSQL**. We'll using <Link href="https://supabase.com">Supabase</Link> as our database provider. Get the **DATABASE_URL** from your Supabase project and add it to your **.env.local** file.
...
Now, let's create the **utils** file. Inside the **drizzle** folder, create a **utils.ts** file. Inside the **utils.ts** file, add the following code.

```ts
// src/lib/drizzle/utils.ts
